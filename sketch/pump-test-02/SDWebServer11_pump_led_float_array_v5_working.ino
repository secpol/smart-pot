...
void setup(void){
  ...
  File conf = SD.open("conf.ini"); //open configuration file from SD card
...
void loop(void){
  ...
  int dly = set[1]; //value to be read from an array
  ...
  if ((unsigned long)(currentMillis - previousMillistemp) >= intervaltemp) {
    ...
    {
      digitalWrite(5, HIGH);
      delay(dly); //value added from an array defines how long pump will be active
      digitalWrite(5, LOW);
      delay(dly); //value added from an array defines how long pump will be inactive
