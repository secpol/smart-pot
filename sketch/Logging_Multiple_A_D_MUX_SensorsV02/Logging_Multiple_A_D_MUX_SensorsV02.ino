...
String loggingMst(){
 ...
 // mst_value = mst_value / 4    // TESTING calibration Scale to 8 bits (0 - 255).
  ...
  mst_value = map(mst_value, 600, 535 ,0, 100);
...
//MST-////////////////////////////////////////////////////////////////////////////////////
int powerON(){      // POWER something ON - OFF connected to Mux channel Y5
  //set the selectPins to set channel y5 HIGH
  pinMode(3,OUTPUT);
  digitalWrite(selPin[0],HIGH);   // a
  digitalWrite(selPin[1],LOW);    // b
  digitalWrite(selPin[2],HIGH);   // c
...
void loop()
{
  ...
  if ((unsigned long)(currentMillis - previousMillisTime) >= intervalTime) {
    ...
    //powerON();               // Testing powering devices with de-Mux function ( LED on channel y5)
 }
